from utils.noniid_partition import non_iid_partition_with_dirichlet_distribution
import pandas as pd
import numpy as np
import torch
...
def image_preprocess_dl(args,data,label_list,test_partition=0.2):
    ...
    for key,client_data in net_dataidx_map.items():
        N_client = len(client_data)#1クライアントに存在するデータの個数
        N_train = int(N_client*(1-test_partition))#そのうち、trainに使うデータ数
